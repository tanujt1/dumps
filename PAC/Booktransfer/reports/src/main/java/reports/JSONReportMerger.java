...
public class JSONReportMerger {
    ...
    public static void main (String[] args) throws Throwable{
    	...
    	System.out.println("JSONReport Merger started..");
        if (reportDirectory.exists()) {
            JSONReportMerger merger = new JSONReportMerger();
            System.setProperty("log.name", "jsonmerger");
            log = LoggerFactory.getLogger(JSONReportMerger.class);
            merger.mergeReports(reportDirectory);
        }
        System.out.println("JSONReport Merger completed..");
    }
    ...
    public void mergeReports(File reportDirectory) throws Throwable {
        ...
        File targetFile = new File(targetReportPath.toString());
        ...
        for (File report : existingReports) {
           ...
           //if (report.getName().equals(reportFileName)) {
                ...
                if (mergedReport == null) {
                	...
                	FileUtils.copyFile(report, targetFile);
                    // access this first copied report
                    mergedReport = targetFile;
                } else {
                    //otherwise merge this report into existing master report
                	System.out.println("Merged.."+mergedReport+"Iter Report:"+report);
                	log.info("Merged.."+mergedReport+"Iter Report:"+report);
